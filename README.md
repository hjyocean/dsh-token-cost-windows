...
- **对话区宽度**：可强制对话区撑满窗口宽度，或切回官方窄列 748px；两者均由插件注入覆盖，不受 DSH 构建产物影响，升级后依然生效。
...
| `chatWidth` | 'wide' | 'default' | `'wide'` | 对话区宽度：`wide` 全宽，`default` 官方窄列 748px（两者均由插件注入覆盖，不受 DSH 构建产物影响） |
